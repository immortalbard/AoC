...
def tractor(code,width,height):
    ...
    for i in range(height):
        for k in range(width):
            image[i][k]=intcode(code,[i,k]).pop()
...
def SantaCheck(code):
    ...
    for y in range(99,10000):
        line=tractor(code,10000,1)
        if sum(line[0])>=100:
            x=line.index(1)
            out100=intcode(code,[y+99,x+99])
            if out100[0]==1:
                return 10000*x+(y),x,y
...
if __name__ == '__main__':
    ...
    # print(sum([image[i].count(1) for i in range(len(image))]))
    # for i in range(height):
    #     print(image[i])
    
    print(SantaCheck(code))